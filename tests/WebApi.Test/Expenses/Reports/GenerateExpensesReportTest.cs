...
public class GenerateExpensesReportTest : CashFlowClassFixture
{
    ...
    {
        var expenseDateFormated = $"{_expenseDate:MM}/{_expenseDate:yyyy}";
        var result = await DoGet(requestUri: $"{METHOD}/pdf?month={expenseDateFormated}", token: _adminToken);
    ...
    {
        var expenseDateFormated = $"{_expenseDate:MM}/{_expenseDate:yyyy}";
        var result = await DoGet(requestUri: $"{METHOD}/excel?month={expenseDateFormated}", token: _adminToken);
